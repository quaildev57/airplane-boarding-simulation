...
class Lobby:
    ...
    def remove_passenger(self, row_num):
        if not self.lobby_rows[row_num].passengers:
            print(f"Error: No passengers left in lobby row {row_num} to remove.")
            return None  # Return None explicitly when no passengers
...
class AirplaneEnv(gym.Env):
    ...
    def step(self, row_num):
        ...
        #Added a new check when there are no passengers left in the lobby row

        if passenger is None:
            # a penalty is returned if no passenger is added to the boarding line

            return self._get_obseravation(), -10, False, False, False, {"info": "No passenger to board from the selected lobby row."}
        self.boarding_line.add_passenger(passenger)
